...
class Net(nn.Module):
    ...
    def forward(self, data_dict, training=False, iter_num=0, epoch=0, labels=None,
        mode="train",
        soft_label=True,
        smooth_label=False,
        filter=0):
        ...
        if training == True:
            ...
            graph1_feat = F.normalize(node_feature_list[0], dim=-1)
            graph2_feat = F.normalize(node_feature_list[1], dim=-1)
            score = graph1_feat @ graph2_feat.T
            
            # loss function   
            criterion = InfoNCELoss()    

            if mode == "predict":
                I = criterion(score, mode="predict")
                p = I.clamp(0, 1)
                return p  

            
            loss, row_mean, col_mean = criterion(score,
                                                 labels=labels,
                                                 mode=mode,
...
class Backbone_ensemble(CNN):
    def __init__(self):
        super(Backbone_ensemble, self).__init__()
        self.message_pass_node_features = SiameseSConvOnNodes(input_node_dim=cfg.COMMON.FEATURE_CHANNEL * 2)
        self.build_edge_features_from_node_features = SiameseNodeFeaturesToEdgeFeatures(
            total_num_nodes=self.message_pass_node_features.num_node_features
        )
    ...
    def forward(self, data_dict, online=True, type="ensemble"):
        with torch.no_grad():
            self.logit_scale.clamp_(0, 4.6052)  # clamp temperature to be between 0.01 and 1
        ...
        if online == False:
            ...
            return unary_affs_list, num_graphs
        elif online == True:
            # output of the online network
            x_list = []
            for unary_affs, (idx1, idx2) in zip(unary_affs_list, lexico_iter(range(num_graphs))):
...
class Net_ensemble(nn.Module):
    def __init__(self):
        ...
        self.onlineNet = Backbone_ensemble()
        self.backbone_params = list(self.onlineNet.backbone_params) # used in train_eval.py
        self.warmup_step = cfg.COMMON.WARMUP_STEP  # warmup steps for the distillation
        self.epoch_iters = cfg.TRAIN.EPOCH_ITERS  # iterations for one epoch, specified by the training dataset
    ...
    def forward(self, data_dict, training=False, iter_num=0, epoch=0, labels=None,
        ...
        # output of the online network
        unary_affs_list, num_graphs = self.onlineNet(data_dict)

        if training == True and False:
            # the momentum network is only using for training
            assert cfg.COMMON.DISTILL == True
            ...
            return unary_affs_list, num_graphs, data_dict
