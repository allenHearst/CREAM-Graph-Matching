    ...
    for i, cls in enumerate(classes):
        ...
        for inputs in dataloaders[i]:
            ...
            if cfg.PROBLEM.TYPE == '2GM':
                ...
                if 'aff_mat' in outputs and False:
                    print("true")
...
def eval_model_ensemble(model_A, model_B, classes, bm, last_epoch=True, verbose=False, xls_sheet=None):
    print('Start evaluation...')
    since = time.time()
    ...
    for i, cls in enumerate(classes):
        ...
        for inputs in dataloaders[i]:
            ...
            with torch.set_grad_enabled(False):
                ...
                unary_affs_list = []
                for j in range(len(unary_affs_list_A[0])):
                ...
                assert num_graphs_A == num_graphs_B
                if num_graphs_A == num_graphs_B:
                    num_graphs = num_graphs_A
...
if __name__ == '__main__':
    ...
    with DupStdoutFileManager(str(Path(cfg.OUTPUT_PATH) / ('eval_log_' + now_time + '.log'))) as _:
        ...
        if cfg.EVAL.EPOCH is not None and cfg.EVAL.EPOCH > 0:
            model_path_A = str(Path(cfg.OUTPUT_PATH) / 'params' / 'params_A_{:04}.pt'.format(20))
            model_path_B = str(Path(cfg.OUTPUT_PATH) / 'params' / 'params_B_{:04}.pt'.format(20))
        # if len(cfg.PRETRAINED_PATH) > 0:
        #     model_path = cfg.PRETRAINED_PATH
        if len(model_path_A) > 0 and len(model_path_B) > 0:
            print('Loading model parameters from {}'.format(model_path_A))
            print('Loading model parameters from {}'.format(model_path_B))
            load_model(model_A, model_path_A, False)
            load_model(model_B, model_path_B, False)

        pcks = eval_model_ensemble(
            model_A, model_B, clss,
            benchmark,
            verbose=True,
            xls_sheet=ws
        )
...
# if __name__ == '__main__':
    ...
    with DupStdoutFileManager(str(Path(cfg.OUTPUT_PATH) / ('eval_log_' + now_time + '.log'))) as _:
        ...
        model_path = ''
        if cfg.EVAL.EPOCH is not None and cfg.EVAL.EPOCH > 0:
            model_path = str(Path(cfg.OUTPUT_PATH) / 'params' / 'params_A_{:04}.pt'.format(cfg.EVAL.EPOCH))
            # model_path = str("/root/workspace/2023-ICCV-COMMON-main/output/vgg16_common_willow/params/params_A_0010.pt")
        if len(cfg.PRETRAINED_PATH) > 0:
            model_path = cfg.PRETRAINED_PATH
        if len(model_path) > 0:
    ...
    with DupStdoutFileManager(str(Path(cfg.OUTPUT_PATH) / ('eval_log_' + now_time + '.log'))) as _:
        ...
        model_path = ''
        if cfg.EVAL.EPOCH is not None and cfg.EVAL.EPOCH > 0:
            model_path = str(Path(cfg.OUTPUT_PATH) / 'params' / 'params_B_{:04}.pt'.format(cfg.EVAL.EPOCH))
            # model_path = str("/root/workspace/2023-ICCV-COMMON-main/output/vgg16_common_willow/params/params_A_0010.pt")
        if len(cfg.PRETRAINED_PATH) > 0:
            model_path = cfg.PRETRAINED_PATH
        if len(model_path) > 0:
